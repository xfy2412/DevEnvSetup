...
| 天气窗口跑到右侧、与右侧窗口重叠 | **Edge 行为**：执行天气手势（F5 → 缩放 → ↓）期间，Edge 会把已经开始最大化的天气窗口**掉出最大化**并重新摆到右侧槽位（实测 `(-14,-14) 3868x2080` → `(2511,23) 1377x2057`，正好压在右侧窗口上）。脚本现在在手势后**立即重钉**一次，压栈后再**兜底重钉**一次。`-Verbose` 会打印 `[rect]` 轨迹，可直接看出偏移发生在哪一步 |
